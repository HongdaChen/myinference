...
class PointDetector(nn.modules.Module):
    ...
    def forward(self, data_dict):
        ...
        points_angle_pred = self.point_predictor(features)  # [b, 4, 16, 16]
        points_pred, angles_pred = (
            points_angle_pred[:, :3, :],
            points_angle_pred[:, 3, :],
        )
        angles_pred = angles_pred.unsqueeze(1)
        points_pred = torch.sigmoid(points_pred)
        angles_pred = torch.tanh(angles_pred)
        points_angle_pred = torch.cat([points_pred, angles_pred], dim=1)
        data_dict["points_angle_pred"] = points_angle_pred
    ...
    def get_targets_points(self, data_dict):
        points_angle_pred = data_dict['points_angle_pred']
        marks_gt_batch = data_dict['marks']
        npoints = data_dict['npoints']

        b, c, h, w = points_angle_pred.shape
        ...
        for batch_idx, marks_gt in enumerate(marks_gt_batch):
            ...
            for marking_point in marks_gt[:n]:
                x, y, angle = marking_point[:3]
                col = math.floor(x * w)
                row = math.floor(y * h)
                # Confidence Regression
                targets[batch_idx, 0, row, col] = 1.0
                # Offset Regression
                targets[batch_idx, 1, row, col] = x * w - col   # 0.0
                targets[batch_idx, 2, row, col] = y * h - row   # 0.0
                # Angle Regression
                targets[batch_idx, 3, row, col] = angle

                mask[batch_idx, 1:4, row, col].fill_(1.0)
        return targets, mask
    ...
    def post_processing(self, data_dict):
        ...
        points_angle_pred_batch = []
        slots_pred = []
        for b, marks in enumerate(points_angle_pred):
            points_angle_pred = get_predicted_points(marks, self.cfg.point_thresh, self.cfg.boundary_thresh)
            points_angle_pred_batch.append(points_angle_pred)
         
            if len(points_angle_pred) > 0:
                points_np = np.concatenate([p[1].reshape(1, -1) for p in points_angle_pred], axis=0)
            else:
                # points_np = np.zeros((self.cfg.max_points, 2))
                points_np = np.zeros((self.cfg.max_points, 3))

            if points_np.shape[0] < self.cfg.max_points:
                # points_full = np.zeros((self.cfg.max_points, 2))
                points_full = np.zeros((self.cfg.max_points, 3))
                points_full[:len(points_angle_pred)] = points_np
            else:
            ...
            for i in range(n):
                for j in range(n):
                    ...
                    if score > 0.5:
                        x1, y1 = points_np[i,:2]
                        x2, y2 = points_np[j,:2]
                        angle1 = points_np[i, 2]
                        angle2 = points_np[j, 2]
                        slot = (score, np.array([x1, y1, x2, y2, angle1, angle2]))
                        slots.append(slot)

            slots_pred.append(slots)

        pred_dicts['points_angle_pred'] = points_angle_pred_batch
    ...
    def get_training_loss(self, data_dict):
        ...
        loss_point = self.point_loss_func(points_angle_pred * mask, targets * mask)
